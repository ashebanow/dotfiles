...
try:
    ...
    from package_analysis import RepologyClient, load_custom_installations, merge_custom_installation
    from package_generators import PlatformDetector, PackageFilter, PackageFileGenerator
...
class TestPackageWorkflowIntegration(unittest.TestCase):
        ...
        self.test_custom_install = {
            "packages": {
                ...
                "custom-only-package": {
                    ...
                    "custom-install": {
                        "default": ["echo 'Installing custom package'"]
                    },
    ...
    def test_complete_package_analysis_workflow(self):
        ...
        self.assertEqual(
            merged_entry["custom-install"]["is_darwin"],
    ...
    def test_complete_package_generation_workflow(self):
        ...
        toml_data = package_analysis.load_toml(Path(__file__).parent / "assets/package_mapping/test_complete_mappings.toml")
        ...
        for platform_config in platforms_to_test:
            with self.subTest(platform=platform_config.get('get_native_package_manager', lambda: 'unknown')()):
            ...
            with self.subTest(platform=scenario['name']):
                ...
                if 'expected_native' in scenario:
                    ...
                    for pkg in expected_native:
                        self.assertIn(pkg, native_packages, 
                ...
                if 'expected_homebrew' in scenario:
                    ...
                    for pkg in scenario['expected_homebrew']:
                        self.assertIn(pkg, homebrew_packages,
                ...
                if 'expected_homebrew_darwin' in scenario:
                    ...
                    for pkg in scenario['expected_homebrew_darwin']:
                        self.assertIn(pkg, homebrew_darwin_packages,
                ...
                if 'expected_flatpak' in scenario:
                    ...
                    for pkg in scenario['expected_flatpak']:
                        self.assertIn(pkg, flatpak_packages,
                ...
                if 'expected_custom' in scenario:
                    ...
                    for pkg in scenario['expected_custom']:
                        self.assertIn(pkg, custom_packages,
