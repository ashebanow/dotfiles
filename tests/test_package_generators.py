...
class TestPackageFilter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_toml = {
            ...
            "custom-fallback": {
                ...
                "brew-supports-darwin": False,
                "brew-supports-linux": False,
                "custom-install": {"default": ["echo 'only option'"]},
    ...
    def test_should_use_native_package(self):
        ...
        self.assertTrue(
            self.filter.should_use_native_package(
                "standard-package", self.test_toml["standard-package"]
            )
        )
        self.assertFalse(
            self.filter.should_use_native_package(
                "flatpak-package", self.test_toml["flatpak-package"]
            )
        )
    ...
    def test_should_use_flatpak(self):
        ...
        self.assertTrue(
            self.filter.should_use_flatpak("flatpak-package", self.test_toml["flatpak-package"])
        )
        self.assertFalse(
            self.filter.should_use_flatpak("standard-package", self.test_toml["standard-package"])
        )

    def test_should_use_custom_install_always(self):
        """Test custom installation with always priority."""
        self.assertTrue(
            self.filter.should_use_custom_install("custom-always", self.test_toml["custom-always"])
        )

    def test_should_use_custom_install_fallback(self):
        """Test custom installation with fallback priority."""
        self.assertTrue(
            self.filter.should_use_custom_install(
                "custom-fallback", self.test_toml["custom-fallback"]
            )
        )
...
class TestPackageFileGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_toml = {
            "regular-package": {"brew-is-cask": False, "description": "Regular package"},
            "cask-package": {"brew-is-cask": True, "description": "Cask package"},
            "aur-package": {"arch-is-aur": True, "description": "AUR package"},
            "regular-arch-package": {"arch-is-aur": False, "description": "Regular Arch package"},
        }

    def test_generate_brewfile(self):
        """Test Brewfile generation."""
        packages = {"regular-package": "regular-pkg", "cask-package": "cask-pkg"}
    ...
    def test_generate_archfile(self):
        ...
        packages = {"aur-package": "aur-pkg", "regular-arch-package": "arch-pkg"}
    ...
    def test_generate_simple_list(self):
        ...
        packages = {"package1": "pkg1", "package2": "pkg2"}
    ...
    def test_generate_customfile(self):
        ...
        test_toml = {
            ...
            "simple-package": {"description": "Simple package"},
        }
