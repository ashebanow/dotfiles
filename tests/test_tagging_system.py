...
try:
    from package_analysis_tagged import (
        analyze_homebrew_data_for_tags,
        analyze_repology_data_for_tags,
        generate_tags_for_package,
        suggest_priority_tags,
        suggest_role_tags,
    )
    from tagged_package_filter import (
        ...
        TaggedPackageFilter,
        TagNamespace,
        auto_categorize_package,
        migrate_package_to_tags,
    )
...
class TestMigrationUtilities(unittest.TestCase):
    ...
    def test_migrate_aur_package(self):
        ...
        entry = {"arch-pkg": "aur-package", "arch-is-aur": True, "description": "AUR package"}
...
class TestTagNamespaces(unittest.TestCase):
    ...
    def test_namespace_validation_in_filter(self):
        ...
        self.assertTrue(any(recognized_tag.startswith(f"{ns.value}:") for ns in TagNamespace))

        # Test with unrecognized namespace
        unrecognized_tag = "custom:tag"
        self.assertFalse(any(unrecognized_tag.startswith(f"{ns.value}:") for ns in TagNamespace))
