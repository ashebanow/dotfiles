...
class TestGitHubActionsIntegration(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            result = subprocess.run(
                ["act", "--version"], capture_output=True, text=True, check=True
            )
    ...
    def test_act_is_working(self):
        ...
        result = subprocess.run(
            ["act", "--list"], cwd=self.project_root, capture_output=True, text=True
        )
        ...
        for workflow_file in self.workflows_dir.glob("*.yml"):
            with self.subTest(workflow=workflow_file.name):
                ...
                if "yaml:" in result.stderr or "workflow is not valid" in result.stderr:
                    self.fail(
                        f"Workflow {workflow_file.name} has YAML syntax errors:\n{result.stderr}"
                    )

                # Docker connection errors are OK for syntax validation
                if "Cannot connect to the Docker daemon" in result.stderr:
                    self.skipTest(
                        f"Docker not available for {workflow_file.name} - YAML syntax is valid"
                    )
    ...
    def test_ci_workflow_dry_run(self):
        ...
        if (
            ...
            or "no basic auth credentials" in result.stderr
            or "pull access denied" in result.stderr
        ):
            self.skipTest(
                "GitHub authentication required for container images - dry-run syntax validation passed"
            )
    ...
    def test_refresh_cache_workflow_dry_run(self):
        ...
        # Should succeed in dry-run mode
        self.assertEqual(
            result.returncode, 0, f"Refresh cache workflow dry-run failed: {result.stderr}"
        )
    ...
    def test_workflow_event_triggers(self):
        ...
        for event in ["push", "pull_request", "workflow_dispatch"]:
            with self.subTest(event=event):
                ...
                self.assertEqual(
                    result.returncode, 0, f"CI workflow should trigger on {event}: {result.stderr}"
                )
...
'''
        ...
        try:
            # Test Python syntax
            result = subprocess.run(
                [sys.executable, "-m", "py_compile", temp_script], capture_output=True, text=True
            )

            self.assertEqual(
                result.returncode, 0, f"Refresh cache script has syntax errors: {result.stderr}"
            )

            # Test that script can be imported without syntax errors
            result = subprocess.run(
                [sys.executable, temp_script, "--help"], capture_output=True, text=True
            )

            # Should either show help or fail gracefully (not with syntax/import errors)
            if (
                "NameError" in result.stderr and "Optional" in result.stderr
    ...
    def test_package_name_mappings_logic(self):
        ...
        for original, mapped in homebrew_mappings.items():
            self.assertNotEqual(
                original, mapped, f"Mapping {original} -> {mapped} should be different names"
            )
    ...
    def test_workflow_secrets_handling(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".secrets", delete=False) as f:
            secrets = {"GITHUB_TOKEN": "fake-token-for-testing"}
        ...
        try:
            ...
            self.assertEqual(
                result.returncode, 0, f"Workflows should handle fake secrets: {result.stderr}"
            )
...
class TestWorkflowEnvironmentCompatibility(unittest.TestCase):
    ...
    def test_python_version_matrix(self):
        ...
        if (
            ...
            self.skipTest(
                "GitHub authentication required for container images - matrix syntax validation passed"
            )
    ...
    def test_ubuntu_runner_compatibility(self):
        ...
        if (
            ...
            self.skipTest(
                "GitHub authentication required for container images - Ubuntu compatibility syntax validation passed"
            )
...
def run_act_tests():
    ...
    for test_class in [
        ...
        TestActConfiguration,
        TestWorkflowEnvironmentCompatibility,
    ]:
