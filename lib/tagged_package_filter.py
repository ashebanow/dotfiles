...
class TaggedPackageFilter:
    ...
    def should_include_package(self, package_name: str, entry: Dict[str, Any], 
                              required_tags: List[str] = None, 
                              excluded_tags: List[str] = None) -> bool:
...
def auto_categorize_package(package_name: str, description: str) -> List[str]:
    ...
    # System tools
    if any(sys in name_lower or sys in desc_lower for sys in ['system', 'monitor', 'process', 'cpu', 'memory']):
        tags.append('cat:system')
    
    # Network tools
    if any(net in name_lower or net in desc_lower for net in ['network', 'http', 'ftp', 'ssh', 'vpn', 'proxy']):
        tags.append('cat:network')
    
    # Security tools
    if any(sec in name_lower or sec in desc_lower for sec in ['security', 'crypt', 'password', 'auth', 'gpg']):
    ...
    if any(virt in name_lower for virt in ['docker', 'container', 'kubernetes', 'k8s', 'vagrant', 'vm']):
    ...
    # Browsers
    if any(browser in name_lower for browser in ['firefox', 'chrome', 'chromium', 'browser', 'surf']):
