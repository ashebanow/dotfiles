...
from lib.tagged_package_filter import migrate_package_to_tags, auto_categorize_package
...
def write_toml(data: Dict[str, Any], filepath: str) -> None:
    ...
    with open(filepath, 'w') as f:
        for package_name in sorted(data.keys()):
            ...
            field_order = [
                'description', 'tags',  # Important fields first
                'arch-pkg', 'arch-is-aur', 'apt-pkg', 'fedora-pkg', 'flatpak-pkg',  # Package managers
                'brew-pkg', 'brew-tap', 'brew-supports-darwin', 'brew-supports-linux', 'brew-is-cask',  # Homebrew
                'prefer_flatpak', 'priority', 'custom-install-priority',  # Priorities
                'custom-install', 'requires-confirmation', 'install-condition'  # Custom install
            ]
...
def analyze_migration(toml_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    legacy_fields = [
        'brew-supports-darwin', 'brew-supports-linux', 'brew-is-cask',
        'arch-is-aur', 'prefer_flatpak'
    ]
...
def migrate_packages(toml_data: Dict[str, Any], 
                    packages: List[str] = None,
                    auto_categorize: bool = True,
                    preserve_legacy: bool = True) -> Dict[str, Any]:
...
def add_role_tags(toml_data: Dict[str, Any], role_mapping: Dict[str, List[str]]) -> Dict[str, Any]:
    ...
    default_role_mapping = {
        'development': [
            ...
            'vim', 'neovim', 'emacs', 'vscode', 'intellij', 'sublime',
            'docker', 'vagrant', 'virtualbox', 'gdb', 'lldb', 'valgrind'
        ],
        'server': [
            'nginx', 'apache', 'postgresql', 'mysql', 'redis', 'memcached',
            'prometheus', 'grafana', 'elasticsearch', 'logstash', 'kibana'
        ],
        'desktop': [
            'firefox', 'chrome', 'slack', 'discord', 'zoom', 'spotify',
            'vlc', 'gimp', 'inkscape', 'libreoffice', 'thunderbird'
        ],
        'security': [
            'gpg', 'pass', 'keepass', 'bitwarden', 'openssl', 'openssh',
            'fail2ban', 'ufw', 'iptables', 'nmap', 'wireshark'
        ],
        'data-science': [
            'jupyter', 'anaconda', 'pandas', 'numpy', 'scikit-learn',
            'tensorflow', 'pytorch', 'r', 'octave', 'matlab'
        ]
    }
...
def main():
    ...
    parser.add_argument(
        '--packages',
        nargs='+',
        help='Specific packages to migrate (default: all)'
    )
    parser.add_argument(
        '--no-auto-categorize',
        action='store_true',
    ...
    parser.add_argument(
        '--add-roles',
        action='store_true',
        help='Add role tags based on package names'
    )
    parser.add_argument(
        '--analyze',
        action='store_true',
        help='Analyze migration status without making changes'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
