...
def show_cache_stats(cache_file: str = '.repology_cache.json', ttl_days: int = 7):
    ...
    if total_entries > 0:
        ...
        print(f"📅 Refresh strategy: Age-based (oldest entries first)")
        
        # Show age distribution
        if fresh_count > 0 and expired_count > 0:
            print(f"🔄 Cache health: {(fresh_count/total_entries)*100:.1f}% fresh entries")

def clean_expired_cache(cache_file: str = '.repology_cache.json', ttl_days: int = 7):
    ...
    for package, entry in cache.items():
        if isinstance(entry, dict) and '_timestamp' in entry:
            ...
            if 'data' in entry and entry['data'] is None:
                ...
                # Check if all platforms are false (indicates API failure)
    ...
    expired_count = len(cache) - len(fresh_cache) - error_entries_removed
...
if __name__ == '__main__':
    ...
    if args.stats_only:
        ...
        clean_expired_cache(args.cache_file, args.ttl_days)