...
def enhance_package_tags(package_name, package_data, cache_data):
    ...
    if (platforms.get('homebrew') or 
        cache_entry.get('brew-supports-darwin') or 
        cache_entry.get('brew-is-cask')):
        new_tags.add('os:macos')
    
    if (platforms.get('arch_official') or platforms.get('arch_aur') or 
        platforms.get('debian') or platforms.get('ubuntu') or 
        platforms.get('fedora') or platforms.get('flatpak')):
    ...
    if not has_cache_data:
        ...
        cask_indicators = [
            'visual-studio-code', 'claude', 'ghostty', 'kitty', 'signal', 
            'zoom', 'zen', 'dolphin', 'parsec', 'pinta'
        ]
        ...
        if ('.' in package_name and 
            (package_name.startswith('com.') or 
             package_name.startswith('org.') or
             package_name.startswith('app.'))):
            new_tags.add('pm:flatpak')
            new_tags.add('os:linux')
        
        # Font packages (usually cross-platform)
        if ('font' in package_name.lower() or 
            package_name.startswith('ttf-') or 
            package_name.startswith('fonts-')):
            new_tags.add('os:linux')
            new_tags.add('os:macos')
...
def print_enhancement_report(enhancements, stats):
    ...
    cask_enhancements = {k: v for k, v in enhancements.items() 
                        if 'pm:homebrew:cask' in v['added_tags']}
    
    if cask_enhancements:
        print(f"🍺 **Cask Enhancements:**")
...
def main():
    ...
    parser.add_argument('--output', default='packages/package_mappings.toml.enhanced',
