...
def print_analysis(stats):
    ...
    print(f"   With any tags: {stats['with_tags']} ({stats['with_tags']/stats['total_packages']*100:.1f}%)")
    print(f"   With OS tags: {stats['with_os_tags']} ({stats['with_os_tags']/stats['total_packages']*100:.1f}%)")
    print(f"   With PM tags: {stats['with_pm_tags']} ({stats['with_pm_tags']/stats['total_packages']*100:.1f}%)")
    ...
    print(f"🍺 **Cask Candidates (from cache analysis):**")
    ...
    for package_name, package_data in toml_data.items():
        ...
        if package_name in cache_data:
            ...
            if (platforms.get('arch_official') or platforms.get('arch_aur') or 
                platforms.get('debian') or platforms.get('ubuntu') or platforms.get('fedora')):
