...
def write_toml(data: Dict[str, Any], filepath: str) -> None:
    ...
    with open(filepath, 'w') as f:
        for section_name, section_data in sorted(data.items()):
            ...
            if not has_packages:
                f.write(f"# Package '{section_name}' not found on any platform - kept for periodic retry\n")
                f.write(f"# This entry will be excluded from generated package files\n")
...
class RepologyClient:
    ...
    def _is_stable_version(self, version: str) -> bool:
        ...
        unstable_patterns = [
            '~a', '~b', '~rc',  # Debian-style
            '-alpha', '-beta', '-rc',  # Standard patterns
            '.dev', '.pre',  # Python development versions
            '-dev', '-devel',  # Development packages
            'git', 'svn',  # VCS versions
    ...
    def query_package(self, package_name: str) -> Optional[Dict[str, Any]]:
        ...
        if repology_name != package_name:
            print(f"\r\033[K🔄 Mapping {package_name} → {repology_name} for Repology...", end="", flush=True)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result and isinstance(result, dict) and result.get('platforms', {}).get('homebrew'):
                ...
                print(f"    Warning: Repology API rate limited (403) for {package_name} - will retry next time")
                # Don't cache 403 errors - they should be retried
                return None
            else:
                print(f"    Warning: Repology API error {response.status_code} for {package_name} - will retry next time")
    ...
    def _score_package_name(self, target_name: str, candidate_name: str, repo: str) -> int:
        ...
        candidate_clean = candidate_lower.replace('lib', '').replace('-dev', '').replace('-devel', '')
        ...
        bad_suffixes = ['-git', '-svn', '-bin', '-stable', '-latest', '-nightly', '-dev', '-devel', '-doc', '-docs']
        ...
        import re
        if re.search(r'-\d+(\.\d+)*$', candidate_lower) and not re.search(r'-\d+(\.\d+)*$', target_lower):
            score -= 250  # Increased penalty for version-specific packages
        
        # Penalty for architecture-specific packages
        arch_patterns = ['-m2', '-arm64', '-x86_64', '-i386', '-amd64', 'avr-', 'mingw-', '-arm-', '-none-eabi']
    ...
    def _select_best_package_name(self, target_name: str, candidates: List[Dict[str, str]]) -> str:
        ...
        best_total_score, best_name, best_repo, best_version, best_name_score, best_status = scored_candidates[0]
        ...
        for total_score, name, repo, version, name_score, status in scored_candidates:
            ...
            if (version_parts and (base_name.lower() == target_name.lower() or 
                                  base_name.lower() == target_base.lower())) or \
               (name.lower() == target_base.lower()):
                ...
                versioned_candidates.append((
                    version_parts, name, repo, version, 
                    name_score + base_name_bonus, repo_score, is_stable, is_vulnerable, status, status_score
                ))
        ...
        if versioned_candidates:
            if target_version:
                ...
                def enhanced_version_score(candidate):
                    ...
                    version_parts, name, repo, version, name_score, repo_score, is_stable, is_vulnerable, status, status_score = candidate
                    ...
                    return (-distance, version_preference, stability_bonus, status_score, repo_score, name_score)
    ...
    def _parse_repology_response(self, data: List[Dict[str, Any]], target_name: str) -> Dict[str, Any]:
        ...
        for entry in data:
            ...
            candidate_data = {'name': package_name, 'repo': repo, 'version': version, 'status': status}
...
class BrewClient:
    ...
    def _query_formula(package_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            macos_platforms = {'arm64_sequoia', 'arm64_sonoma', 'arm64_ventura', 'arm64_monterey', 
                             'sonoma', 'ventura', 'monterey', 'big_sur', 'catalina'}
...
def generate_package_entry(package_name: str, repology_client: RepologyClient, 
                         existing_toml: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
...
def _select_best_alias_result(alias_results: List[Tuple[str, Dict[str, Any]]]) -> Tuple[str, Dict[str, Any]]:
...
def generate_complete_toml(package_lists: List[str], specific_packages: List[str] = None,
    ...
    if specific_packages:
        ...
        valid_existing_packages = {pkg for pkg in existing_toml.keys() if is_valid_package_name(pkg)}
    ...
    for i, package_name in enumerate(sorted(all_packages), 1):
        ...
        progress_bar = "▓" * (i * 20 // len(all_packages)) + "░" * (20 - (i * 20 // len(all_packages)))
