                        ...
                        suggestions.append(
                            {
                                ...
                                "add_field": type_key,
                                "add_value": package,
                                "reason": "Similar to existing package",
                            }
                        )
...
def check_arch_package(package_name):
    ...
    try:
        ...
        result = subprocess.run(
            ["pacman", "-Si", package_name], capture_output=True, text=True, timeout=10
        )
        ...
        # Try yay for AUR
        result = subprocess.run(
            ["yay", "-Si", package_name], capture_output=True, text=True, timeout=10
        )
        if result.returncode == 0:
...
def check_apt_package(package_name):
    ...
    try:
        result = subprocess.run(
            ["apt", "show", package_name], capture_output=True, text=True, timeout=10
        )
...
def check_fedora_package(package_name):
    ...
    try:
        result = subprocess.run(
            ["dnf", "info", package_name], capture_output=True, text=True, timeout=10
        )
        return result.returncode == 0
...
def parse_repology_response(data):
    ...
    package_names = {"arch": None, "apt": None, "fedora": None, "flatpak": None}
...
def check_flatpak_package(package_id):
    ...
    try:
        result = subprocess.run(
            ["flatpak", "info", package_id], capture_output=True, text=True, timeout=10
        )
        return result.returncode == 0
...
def get_homebrew_platform_support(package_name):
    ...
    try:
        ...
        is_cask = "/cask" in tap or tap == "homebrew/cask" or "cask" in tap.split("/")

        # If tap detection is unclear, try direct cask command as fallback
        if not is_cask and not tap:
            try:
                cask_result = subprocess.run(
                    ["brew", "info", "--cask", package_name], capture_output=True, timeout=5
                )
        ...
        supports_darwin = any(
            platform
            for platform in bottle_files.keys()
            if any(
                mac_arch in platform
                for mac_arch in ["arm64_", "sonoma", "ventura", "monterey", "big_sur"]
            )
        )

        supports_linux = any(platform for platform in bottle_files.keys() if "linux" in platform)
...
def generate_new_toml_entries(
    new_packages_by_type, existing_toml, use_repology=False, repology_cache="repology_cache.json"
):
    ...
    for pkg_type, packages in new_packages_by_type.items():
        for package in packages:
            ...
            if "homebrew" in available_platforms:
                ...
                supports_darwin, supports_linux, is_cask = get_homebrew_platform_support(
                    homebrew_name
                )
                ...
                if supports_darwin is not None:
                    ...
                    print(
                        f"    Warning: Could not query Homebrew for {homebrew_name}, using defaults"
                    )
                ...
                if use_repology:
                    ...
                    if repology_data:
                        ...
                        if pkg_names["arch"] and not new_entry.get("arch-pkg"):
                            ...
                            print(
                                f"    Found Arch package: {pkg_names['arch']} (AUR: {platforms['arch_aur']})"
                            )
            ...
            if pkg_type in type_mapping:
                ...
                if use_repology:
                    ...
                    if exists is not None:
                        ...
                        print(
                            f"    Warning: Could not verify Arch package '{package}' (pacman/yay not available)"
                        )
                ...
                if other_type in type_mapping:
                    ...
                    if other_type == "arch" and platform_info["arch_like"]:
                        ...
                        if exists is False:
                            ...
                            print(
                                f"    Warning: Could not verify APT package '{other_pkg}' (apt not available)"
                            )
                        ...
                        if exists is False:
                            ...
                            print(
                                f"    Warning: Could not verify Fedora package '{other_pkg}' (dnf not available)"
                            )
                    elif other_type == "flatpak":
                        # Flatpak is only available on Linux platforms
                        if not platform_info["darwin"] and any(
                            platform_info[k] for k in ["arch_like", "debian_like", "fedora_like"]
                        ):
                            ...
                            if exists is False:
                                ...
                                print(
                                    f"    Warning: Could not verify Flatpak '{other_pkg}' (flatpak not available)"
                                )
...
def main():
    ...
    if args.use_repology:
        ...
        enhanced_entries = enhance_existing_entries_with_repology(
            existing_toml, args.repology_cache
        )
    ...
    if suggestions:
        for suggestion in suggestions:
            if suggestion["action"] == "add_to_existing":
                print(
                    f"✓ Add {suggestion['add_field']} = \"{suggestion['add_value']}\" to [{suggestion['existing_homebrew']}]"
                )
    ...
    print(
        "      Empty strings indicate the package is deliberately not available on that platform."
    )
    print(
        "      Platform-specific fields (like arch-is-aur, brew-is-cask) only appear when applicable."
    )
    print()
    ...
    new_entries = generate_new_toml_entries(
        new_packages_by_type, existing_toml, args.use_repology, args.repology_cache
    )
    ...
    print(
        f"Can be consolidated: {len([s for s in suggestions if s['action'] == 'add_to_existing'])}"
    )
