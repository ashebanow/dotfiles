...
try:
    from tagged_package_filter import TaggedPackageFilter, TagExpression
except ImportError:
...
class EnhancedPlatformDetector:
    ...
    def _detect_atomic_distro(self):
        ...
        try:
            with open('/etc/os-release', 'r') as f:
                os_release = f.read().lower()
                atomic_names = [
                    'silverblue', 'kinoite', 'sericea', 'onyx',  # Fedora Atomic variants
                    'bazzite', 'bluefin',                        # Universal Blue variants
                    'fedora atomic', 'fedora cosmic atomic', 'fedora sway atomic',
                    'microos',                                   # openSUSE MicroOS
                    'immutable', 'atomic'                        # Generic indicators
                ]
    ...
    def supports_homebrew(self) -> bool:
        ...
        return self.is_darwin or (self.is_linux and (
            os.path.exists('/home/linuxbrew/.linuxbrew') or
            os.path.exists('/opt/homebrew')
        ))
    ...
    def detect_desktop_environment(self) -> Optional[str]:
        ...
        if any('gnome' in env for env in [desktop_session, xdg_current_desktop, gdmsession]):
            return "gnome"
        elif any('kde' in env or 'plasma' in env for env in [desktop_session, xdg_current_desktop, gdmsession]):
            return "kde"
        ...
        try:
            import subprocess
            result = subprocess.run(['pgrep', '-f'], capture_output=True, text=True, input='gnome-shell|plasmashell|xfce4-panel|Hyprland|sway|i3|niri')
...
class TaggedPackageFileGenerator:
    ...
    def generate_files_for_current_platform(self) -> Dict[str, str]:
        ...
        if self.platform.supports_homebrew():
            if self.platform.is_darwin:
                ...
                if cask_packages:
                    files['Brewfile-darwin'] = self._generate_brewfile(cask_packages, casks_only=True)
    ...
    def _generate_brewfile(self, packages: Dict[str, Any], 
                          include_casks: bool = True, 
                          casks_only: bool = False) -> str:
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate package files using tag-based filtering'
    )
    
    # Input/output options
    parser.add_argument('--toml', '-t', required=True,
    ...
    parser.add_argument('--role',
                       help='Generate files for specific machine role (e.g., desktop, server)')
    parser.add_argument('--category',
                       help='Generate files for specific category (e.g., development, multimedia)')
    parser.add_argument('--desktop-environment', '--de',
                       help='Generate files for specific desktop environment (e.g., gnome, kde, hyprland)')
    parser.add_argument('--exclude-desktop-environments', '--exclude-de', nargs='+',
                       help='Exclude packages for specific desktop environments')
    parser.add_argument('--atomic-distro', action='store_true',
                       help='Generate files optimized for atomic/immutable distributions')
    parser.add_argument('--traditional-distro', action='store_true',
    ...
    parser.add_argument('--list-tags', action='store_true',
                       help='List all unique tags in the TOML file')
    parser.add_argument('--analyze', action='store_true',
                       help='Analyze package distribution by tags')
    ...
    if args.list_tags:
        ...
        print("=== All Tags ===")
        for namespace in ['os', 'arch', 'dist', 'disttype', 'de', 'pm', 'cat', 'role', 'priority', 'scope']:
    ...
    if args.query:
        ...
        filtered_packages = generator.filter.get_packages_for_desktop_environment(args.desktop_environment)
        files = generator._generate_files_for_packages(filtered_packages)
    elif args.exclude_desktop_environments:
        # Filter excluding desktop environments
        filtered_packages = generator.filter.get_packages_excluding_desktop_environments(args.exclude_desktop_environments)
        files = generator._generate_files_for_packages(filtered_packages)
