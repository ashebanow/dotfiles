...
class PackageFilter:
    ...
    def should_use_homebrew(self, package_name: str, entry: Dict[str, Any]) -> bool:
        ...
        return not (
            self.should_use_custom_install(package_name, entry)
            or self.should_use_native_package(package_name, entry)
            or self.should_use_flatpak(package_name, entry)
        )
    ...
    def should_use_custom_install(self, package_name: str, entry: Dict[str, Any]) -> bool:
        ...
        if custom_priority == "never":
            ...
            has_homebrew = entry.get("brew-supports-darwin", False) or entry.get(
        ...
        for package_name, entry in self.toml_data.items():
            if target == "native":
                ...
                if self.platform.is_darwin:
                    ...
                    darwin_only = entry.get("brew-supports-darwin", False) and not entry.get(
                        "brew-supports-linux", False
                    )
...
class PackageFileGenerator:
    ...
    def generate_brewfile(
        ...
        if original_brewfile and os.path.exists(original_brewfile):
            with open(original_brewfile) as f:
                for line in f:
                    line = line.strip()
...
def should_regenerate_files(toml_path: str, output_dir: str, original_brewfile: str = None) -> bool:
...
def generate_package_files(
    toml_path: str, output_dir: str = None, original_brewfile: str = None, force: bool = False
) -> Dict[str, str]:
    ...
    if (
        not force
        and output_dir
        and not should_regenerate_files(toml_path, output_dir, original_brewfile)
    ):
    ...
    print(
        f"Filtered packages: Native={len(native_packages)}, Flatpak={len(flatpak_packages)}, Homebrew={len(homebrew_packages)}, Custom={len(custom_packages)}"
    )
    ...
    if homebrew_darwin_packages and platform.is_darwin:
        brewfile_darwin_content = generator.generate_brewfile(
            ...
            None,
            None,
            include_overrides=False,  # No taps or overrides for darwin-specific file
        )
...
def main():
    ...
    parser.add_argument("--original-brewfile", help="Original Brewfile.in for preserving taps")

    # Output options
    parser.add_argument("--output-dir", "-o", help="Directory to write generated package files")
    parser.add_argument(
        "--print-only",
        action="store_true",
    ...
    parser.add_argument(
        "--target",
        choices=["native", "flatpak", "homebrew", "homebrew-darwin", "custom", "all"],
        default="all",
