...
- **attribution key / virtual key (VK)** — a per-client bifrost gateway
  ...
  **production secret**: the value is randomly generated by definition, held in
  ...
  `bifrost-config.json`, the client via chezmoi's `bitwardenSecrets` at apply
  time. *Attribution* is not *authorization*: these keys are recorded while
  `enforce_auth_on_inference` is `false`, so they label traffic without
  admitting or rejecting any of it. Rotating one therefore requires a gateway
  redeploy **and** a chezmoi re-apply on each client host.
